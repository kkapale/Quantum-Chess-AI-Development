...
class MinimalQuantumChessGameState():
    ...
    def do_move(self, move_str):
        ...
        _ , move_code = newQCgame.do_move(move_str)
        move_history = newQCgame.get_history()
        self.move_history, self.move_code = move_history, move_code
        #new_game_state = MinimalQuantumChessGameState(init_state, move_history, move_code)
        newQCgame.delete_game()
        #gc.collect()
        return self
    ...
    def __copy__(self):
        init_state, move_history, move_code = self.initial_state_fen, self.move_history, self.move_code
        new_game_state = MinimalQuantumChessGameState(init_state, move_history, move_code)
        return new_game_state
    ...
    def is_game_over(self):
        if int(self.move_code) in [2, 3, 4, 5]:
            return True
        else:
            return False
