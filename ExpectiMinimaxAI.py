from QuantumChessGame import format_moves, number_to_piece
...
class ExpectiMiniMaxAI:
    def __init__(self, use_alpha_beta_pruning=True,
        ...
        self.search_quantum_moves = search_quantum_moves
    ...
    def negamax(self, game, maxdepth, plyFromRoot, alpha = -np.inf, beta = np.inf):
        """
        ...
        if (plyFromRoot > 0):
            ...
            alpha = np.max([alpha, -self.immediateMateValue + plyFromRoot])
            beta = np.min([beta, self.immediateMateValue - plyFromRoot])
            if (alpha >= beta):
...
#         if maxdepth == 3:
#             valid_moves = self.order_moves(gamedata, valid_moves, True)[::-1]
#             valid_move_str = format_moves(valid_moves)
#             print("Ordered moves in first step")
#             for move in valid_move_str:
#                 print(move)
        ...
        for i, move in enumerate(valid_moves):
            ...
            # Skip quantum moves if config says so
            if( not self.search_quantum_moves and '^' in valid_move_str[i] ):
                continue
            ...
            if valid_moves[i]['variant'] == 3:
                ...
                if (np.ceil(1/source_square_probability) == 1) and (np.ceil(1/target_square_probability) == 1):
                    num_repetitions = 1
                ...
                probability_estimates = np.array([occurences[0], occurences[1]])/num_repetitions
                if( np.abs(results[0]) != np.inf ):
                    average_value += probability_estimates[0]*results[0]
                if( np.abs(results[1]) != np.inf ):
            ...
            #if( maxdepth == 3 ):
            #    print("move score: %s = %d"%(valid_move_str[i], average_value))
    ...
    def order_moves(self, gamedata, moves, verbose=False):
        """
        ...
        for i, move in enumerate(moves):
            ...
            if move['promotion_piece'] != 0:
                score += piece_values[number_to_piece[move['promotion_piece']]]
            ...
            if( verbose ):
                print("Move order score: %s = %d"%(format_moves([move]), score))

            scores.append(score)

        # Sort
        indices = np.argsort(scores)[::-1]
        moves = np.array(moves)[indices]
        return moves
    ...
    def minimax(self, game, maximizing, maxdepth, plyFromRoot, alpha = -np.inf, beta = np.inf, white_is_maximizing=True, move_ordering=False):
        ...
        #
        #     indices = np.argsort(scores)[::-1]
        #     valid_moves = np.array(valid_moves)[indices]
        #     valid_move_str = format_moves(valid_moves)
        ...
        for i, move in enumerate(valid_moves):
            ...
            if valid_moves[i]['variant'] == 3:
                ...
                num_repetitions = np.ceil(1/source_square_probability) * np.ceil(1/target_square_probability)
